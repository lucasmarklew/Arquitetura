...
var EstacionamentoService = (function () {
    function EstacionamentoService(http) {
        this.http = http;
        this.urlServico = "http://localhost:8080/ExemploRest/rest/estacionamento";
    }
    EstacionamentoService.prototype.listar = function () {
        return this.http.get(this.urlServico).map(function (res) { return res.json(); });
    };
    EstacionamentoService.prototype.cadastrar = function (estacionamento) {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        var body = JSON.stringify(estacionamento);
        return this.http.post(this.urlServico, body, options).map(function (res) { return res.text(); });
    };
    EstacionamentoService.prototype.atualizar = function (estacionamento) {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        var body = JSON.stringify(estacionamento);
        return this.http.put(this.urlServico, body, options).map(function (res) { return res.text(); });
    };
    EstacionamentoService.prototype.excluir = function (id) {
        var url = this.urlServico + '/' + id;
        return this.http.delete(url).map(function (res) { return res.text(); });
    };
    EstacionamentoService.prototype.buscarPorId = function (id) {
